...
def getCategories():
    ...
    CategorySiteID_elem.text = str(category_site_id)
    
    detailLevel_elem = etree.SubElement(root, "DetailLevel")
    detailLevel_elem.text = detail_level
    
    levelLimit_elem = etree.SubElement(root, "LevelLimit")
    levelLimit_elem.text = str(level_limit)

    output_xml = etree.tostring(root, pretty_print=True, xml_declaration=True, encoding="utf-8")
    print("output_xml: {}".format(output_xml))
